...
def get_hash_scanning(writer):
    ...
    timestamp_long = str(time.time())
    random_number = str(random.randrange(10000000, 99999999))
    
    # Concatenate the components with "-"
    tmp_original = "-".join([package_name, file_sha256, timestamp_long, random_number])
    
    # Encode the concatenated string into bytes using UTF-8 encoding
    tmp_original_bytes = tmp_original.encode('utf-8')
    
    # Calculate the SHA-512 hash
    tmp_hash = hashlib.sha512(tmp_original_bytes).hexdigest()
...
def get_hash_exception(writer):
    ...
    analyze_error_id = writer.getInf("analyze_error_id", "err")
    file_sha256 = writer.getInf("file_sha256", "sha256")
    timestamp_long = str(time.time())
    random_number = str(random.randrange(10000000, 99999999))
    
    # Concatenate the components with "-"
    tmp_original = "-".join([analyze_error_id, file_sha256, timestamp_long, random_number])
