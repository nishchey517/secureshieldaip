...
def random_string():
    return random.choice(string.ascii_letters) + ''.join([random.choice(string.ascii_letters + string.digits) for _ in range(10 - 1)])
...
def is_android_raw(raw):
    ...
    if raw[0:2] == b"PK" or (b'AndroidManifest.xml' in raw and b'META-INF/MANIFEST.MF' in raw):
        val = "APK"
    elif raw[0:3] == b"dex":
        val = "DEX"
    elif raw[0:3] == b"dey":
